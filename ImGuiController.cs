...
namespace DebugGui
{
    public class ImGuiController
    {
        ...
        public void RecreateFontDeviceTexture(bool sendToGPU)
        {
            ...
            if (sendToGPU)
            {
                IntPtr fontTexID = DebugGuiHook.GenerateImGuiFontTexture(pixels, width, height, bytesPerPixel);
        ...
        public void SetPerFrameImGuiData(float deltaSeconds, int _windowWidth, int _windowHeight, System.Numerics.Vector2 _scaleFactor)
        {
			...
			if (deltaSeconds < 0.005f)
				deltaSeconds = 1.0f / 60.0f; // 60 fps
        ...
        public void Render()
        {
            if (_frameBegun)
            {
                ...
                DebugGuiHook.SendImGuiDrawCommands(d);
            }
        }
